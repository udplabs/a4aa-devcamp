...
import { Tenant, BootstrapResponse } from "./tenant";
...
class TenantResolver {
  ...
  private byIssuer = new Map<string, Tenant>();
  private serviceToken: { token: string; expiresAt: number } | null = null;
  ...
  extractDemoName(host?: string): string | null {
    if (!host) return null;
  ...
  private async bootstrap(demoName: string): Promise<BootstrapResponse> {
  ...
  async resolve(demoName: string | null): Promise<Tenant> {
