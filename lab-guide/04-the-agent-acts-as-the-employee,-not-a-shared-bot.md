...
Nexus needs to be able to log document activity to the CRM under the employee's identity. 

A bot token here creates real operational risk: 
- There's one blast radius for every user
- There's no audit trail tied to the individual
- Each time someone leaves, you *need to* manually rotate those credentials.

The right solution for this is **Token Vault**. 

1. Auth0 will store each user's federated CRM credential and at tool-call time.
2. Nexus then asks the vault (politely) for a short-lived, per-user access token scoped to the job at hand.
3. Nexus calls the CRM with that token, and discards it.
...
> [!WARNING]
> If you restart your Codespace, it gets a new public URL. The CRM connection registered in Auth0 will point to the old URL and the live Token Vault path will fail. To fix this, reset your tenant using the reset button on the bottom right of the application and click **Provision Resources** again from the Nexus setup screen to re-register the connection with the new URL.
...
One turned on, Auth0 will automatically request a refresh token from the CRM on every OAuth2 flow so it can maintain the stored credential without user re-authentication.*

![CRM connection Purpose section with Token Vault option selected](images/03-token-vault-purpose-enabled.png)

**Before you enable it**: the vault falls back to an in-memory mock CRM token, so the tool call still succeeds, but Auth0 is not yet involved in storing the credential. 
...
That flow runs against Auth0's My Account API, so the API itself needs to be activated on your tenant, and the SPA needs to be authorized to request a token for it.
...
6. Click **Grant Access**. It takes effect immediately. A separate **Save** button on this screen may appear greyed out. If so, there's nothing left to save and you can move on.
...
> [!NOTE]
> If Token Vault is not yet enabled on the connection (or the user has not linked one), the vault falls back to an in-memory token with the same API shape, so **log_crm_activity** still completes offline. 
...
If either condition is not met, it falls back to the in-memory mock so the lab runs offline.
...
An OAuth2 authorization server and CRM activities API runs on port 3002 in your Codespace. 
